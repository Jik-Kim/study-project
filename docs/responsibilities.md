...
OpenCV 프레임 위에 객체 위치, 검출 여부, 추적 상태와 디버깅 정보를 그려 넣는다.
`ui`가 최종 화면에 표시할 이미지를 만드는 역할까지만 담당하며, 창이나 위젯을
직접 구성하지 않는다.

### `ui`

Tkinter로 통합 실행 창을 조립하고 카메라·상태·시뮬레이션·토픽 흐름·속도 그래프
패널을 배치한다. `visualization`이 만든 프레임과 `nodes`가 전달하는 상태값을
입력받아 표시만 하며, 검출이나 오버레이 계산은 하지 않는다.
