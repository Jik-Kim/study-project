...
- 제어 명령은 RELIABLE, 카메라 영상은 BEST_EFFORT QoS를 사용한다.
...
- `error_x`, `error_y`는 화면 중심과 공 중심 사이의 픽셀 오차다.
- `area`는 검출된 공의 픽셀 면적이며, 면적이 클수록 거북이의 전진
  속도를 높인다.
...
- 토픽 이름과 노드별 Publisher/Subscriber 계약은 ROS2 통신 및
  인터페이스 담당자가 정리하고 팀 공유 후 확정한다.
- 제어 명령 토픽은 RELIABLE QoS를 사용한다.
- 카메라 영상 토픽은 BEST_EFFORT QoS를 사용한다.
...
- HSV 범위와 최소 검출 면적은 ROS2 파라미터로 관리한다.
- 파라미터 초깃값은 실제 카메라와 조명 환경에서 보정한다.
...
- 노드별 구체적인 토픽 이름과 카메라·추적 결과 외 토픽의 QoS
- `error_x`, `error_y`의 정확한 부호 규칙과 제어 식
- 면적을 속도로 변환하는 gain, 최대 속도, 최소 면적 값
- Gazebo의 최종 발표 포함 여부
